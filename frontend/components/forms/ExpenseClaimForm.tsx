...
import { useCreateExpenseClaim, useMePermissions, useSystemInfo, useUpdateExpenseClaim } from '@/lib/queries';
import { problemToast } from '@/lib/api';
import { bangkokToday } from '@/lib/utils';
import type { ExpenseClaimDetail } from '@/lib/types';

// Cycle C (specs/expense-claims.md §5) — shared multi-line create/edit form. Clones payment-vouchers/new's
...
export function ExpenseClaimForm({ edit }: { edit?: ExpenseClaimDetail } = {}) {
  ...
  const [employeeId, setEmployeeId] = useState<number | null>(edit?.employeeId ?? null);
  const [claimDate, setClaimDate] = useState(edit?.claimDate ?? bangkokToday());
  const [title, setTitle] = useState(edit?.title ?? '');
  const [notes, setNotes] = useState(edit?.notes ?? '');
  const [businessUnitId, setBusinessUnitId] = useState<number | null>(edit?.businessUnitId ?? null);
  const [rows, setRows] = useState<Row[]>(
    edit?.lines.map((l) => ({
      key: l.expenseClaimLineId,
  ...
  useEffect(() => {
    ...
    setClaimDate(edit.claimDate);
    setTitle(edit.title ?? '');
    setNotes(edit.notes ?? '');
    setBusinessUnitId(edit.businessUnitId ?? null);
    setRows(edit.lines.map((l) => ({
      key: l.expenseClaimLineId,
      expenseCategoryId: l.expenseCategoryId,
      description: l.description,
      expenseDate: l.expenseDate ?? '',
  ...
  async function saveDraft() {
    ...
    try {
      ...
      if (isEdit && edit) {
        await update.mutateAsync({ id: edit.expenseClaimId, req: payload });
        toast.success(tc('save'));
        router.push(`/expense-claims/${edit.expenseClaimId}`);
        return;
      }
  ...
  return (
    <>
      <PageHeader title={isEdit ? t('editTitle') : t('create')} />
      <div className="max-w-4xl space-y-5">
        ...
        <div className="flex justify-end gap-2">
          <button type="button" className="btn btn-ghost" onClick={() => router.push(isEdit && edit ? `/expense-claims/${edit.expenseClaimId}` : '/expense-claims')} disabled={busy}>
            {tc('cancel')}
          </button>
