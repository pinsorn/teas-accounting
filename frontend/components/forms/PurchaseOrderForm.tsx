...
// WP3 3.3 (D2) — `edit` prop reuses this form for /purchase-orders/[id]/edit
// (Draft-only; saves via PUT and returns to the detail page), mirroring
// QuotationForm's create/edit split.
...
// Read-DTO lines carry no discountPercent (only the post-discount lineAmount) —
// reconstruct it from quantity*unitPrice vs lineAmount, same approach the VI
// PO-pull already uses to reconstruct a line's effective VAT rate.
function toLine(l: PoLineDto): LineItem {
  const gross = l.quantity * l.unitPrice;
  const discountPercent = gross > 0 ? Math.round((1 - l.lineAmount / gross) * 10000) / 100 : 0;
  return {
    ...
    uomText: l.uomText ?? undefined,
    discountPercent,
  };
}

export function PurchaseOrderForm({ edit }: { edit?: PurchaseOrderDetail } = {}) {
  ...
  const {
    ...
    defaultValues: edit
      ? { vendorId: edit.vendorId, lines: edit.lines.map(toLine) }
      : { vendorId: 0, lines: [{ ...EMPTY_LINE }] },
  });

  // Re-hydrate if the edited PO arrives/changes after first render (mirrors QuotationForm).
  useEffect(() => {
    if (!edit) return;
    ...
    setNotes(edit.notes ?? '');
    setVendorLabel(edit.vendorName ?? '');
  ...
  async function submit(v: FormValues) {
    ...
    try {
      ...
      if (isEdit && edit) {
        await update.mutateAsync({ id: edit.purchaseOrderId, req: payload });
        toast.success(tc('save'));
        router.push(`/purchase-orders/${edit.purchaseOrderId}`);
        return;
      }
  ...
  const cancelHref = isEdit && edit ? `/purchase-orders/${edit.purchaseOrderId}` : '/purchase-orders';
        ...
        <SectionCard number={2} title={tcr('docInfo')}>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <label className="form-control">
              ...
              />
              <span className="label-text-alt text-base-content/50">= {formatDateBE(expected)}</span>
            </label>
