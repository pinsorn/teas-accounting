#!/bin/bash
# FE deploy v1.22.0 — VAT-round findings fix: employees modal opening-YTD section (F-3),
...
# package.json + pnpm-lock UNCHANGED vs v1.21.5 -> NO pnpm install. Full
# `git archive v1.22.0 frontend` overlay (--strip-components=1). Rebuild, restart, auto-rollback.
set -uo pipefail
...
tar xf /tmp/fe-src-v1220.tar --strip-components=1
echo "overlaid: $(tar tf /tmp/fe-src-v1220.tar | wc -l) entries"
...
node node_modules/next/dist/bin/next build > /tmp/fe-build-v1220.log 2>&1
RC=$?
if [ $RC -ne 0 ]; then
  echo "BUILD_FAILED rc=$RC -- rolling back"
  tail -30 /tmp/fe-build-v1220.log
