#!/bin/bash
# API deploy v1.22.0 — VAT-round findings fix (spec fix-vat-round-findings.md):
...
EXPECT_TOTAL=$((PREDEPLOY_TOTALSCRIPTS + 2))
echo "PREDEPLOY_TOTALSCRIPTS=$PREDEPLOY_TOTALSCRIPTS (expect $EXPECT_TOTAL post-deploy)"

echo "== backup DB =="
TS=$(date +%Y%m%d-%H%M%S)
sudo -u postgres pg_dump teas | gzip > ~/backups/teas-pre-v1.22.0-deploy-$TS.sql.gz
gunzip -t ~/backups/teas-pre-v1.22.0-deploy-$TS.sql.gz && echo "DB_BACKUP_OK $(ls -la ~/backups/teas-pre-v1.22.0-deploy-$TS.sql.gz | awk '{print $5}')" || { echo "DB_BACKUP_FAILED -- abort"; exit 1; }

echo "== stage =="
rm -rf unpacked.new; mkdir unpacked.new
tar xf /tmp/teas-api-1.22.0-sc.tar -C unpacked.new
...
[ "$VERSION" = "1.22.0" ] && echo "PASS version=$VERSION" || echo "FAIL version=$VERSION"
...
[ "$COGSMAP" = "$NCOMP" ] && echo "PASS cogs_category_remap=$COGSMAP/$NCOMP" || echo "FAIL cogs_category_remap=$COGSMAP (companies=$NCOMP)"

if [ "$HTTP" = "200" ] && [ "$ST" = "online" ] && [ "$PDF" = "404" ] && [ "$PAYROLL" = "401" ] && [ "$VERSION" = "1.22.0" ] && [ "$TOTALSCRIPTS" = "$EXPECT_TOTAL" ] && [ "$YTDCOLS" = "4" ] && [ "$COGS" = "$NCOMP" ] && [ "$COGSMAP" = "$NCOMP" ]; then
