#!/bin/bash
# FE deploy v1.22.10 — finding batch WP1 (route-guard deny on 16 /new routes + CN/DN buttons +
...
# `git archive v1.22.10 frontend` overlay (--strip-components=1). Rebuild, restart, auto-rollback.
...
tar xf /tmp/fe-src-v12210.tar --strip-components=1
echo "overlaid: $(tar tf /tmp/fe-src-v12210.tar | wc -l) entries"
for f in 'app/(dashboard)/expense-claims/new/page.tsx' \
...
grep -q 'noAccessTitle' 'app/(dashboard)/expense-claims/new/page.tsx' || { echo "CONTENT_CHECK_FAILED wp1-gate -- abort"; rm -rf .next; mv .next.old .next; exit 1; }
...
node node_modules/next/dist/bin/next build > /tmp/fe-build-v12210.log 2>&1
RC=$?
if [ $RC -ne 0 ]; then
  echo "BUILD_FAILED rc=$RC -- rolling back"
  tail -30 /tmp/fe-build-v12210.log
  mv .next .next.broken-v12210; mv .next.old .next
