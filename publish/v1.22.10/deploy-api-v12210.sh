#!/bin/bash
# API deploy v1.22.10 — CRIT-1 round-2 fix: NumberedDocumentWriter off-by-one retry cap +
...
echo "== backup DB =="
TS=$(date +%Y%m%d-%H%M%S)
sudo -u postgres pg_dump teas | gzip > ~/backups/teas-pre-v1.22.10-deploy-$TS.sql.gz
gunzip -t ~/backups/teas-pre-v1.22.10-deploy-$TS.sql.gz && echo "DB_BACKUP_OK $(ls -la ~/backups/teas-pre-v1.22.10-deploy-$TS.sql.gz | awk '{print $5}')" || { echo "DB_BACKUP_FAILED -- abort"; exit 1; }

echo "== stage =="
rm -rf unpacked.new; mkdir unpacked.new
tar xf /tmp/teas-api-1.22.10-sc.tar -C unpacked.new
...
[ "$VERSION" = "1.22.10" ] && echo "PASS version=$VERSION" || echo "FAIL version=$VERSION"
...
if [ "$HTTP" = "200" ] && [ "$ST" = "online" ] && [ "$PDF" = "404" ] && [ "$TI" = "401" ] && [ "$VERSION" = "1.22.10" ] && [ "$TOTALSCRIPTS" = "$EXPECT_TOTAL" ]; then
  ...
  mv unpacked unpacked.broken-v12210; mv unpacked.old unpacked
