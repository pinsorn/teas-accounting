#!/bin/bash
# API deploy v1.22.3 — CN/DN list shows referenced TI doc number (server JOIN,
# no N+1) + draft-only DELETE endpoint for CN/DN (mirrors Quotation/BillingNote
# DeleteDraftAsync pattern; gated by existing sales.credit_note.create /
# sales.debit_note.create -- no new permission code).
...
echo "== backup DB =="
TS=$(date +%Y%m%d-%H%M%S)
sudo -u postgres pg_dump teas | gzip > ~/backups/teas-pre-v1.22.3-deploy-$TS.sql.gz
gunzip -t ~/backups/teas-pre-v1.22.3-deploy-$TS.sql.gz && echo "DB_BACKUP_OK $(ls -la ~/backups/teas-pre-v1.22.3-deploy-$TS.sql.gz | awk '{print $5}')" || { echo "DB_BACKUP_FAILED -- abort"; exit 1; }

echo "== stage =="
rm -rf unpacked.new; mkdir unpacked.new
tar xf /tmp/teas-api-1.22.3-sc.tar -C unpacked.new
...
[ "$VERSION" = "1.22.3" ] && echo "PASS version=$VERSION" || echo "FAIL version=$VERSION"
...
if [ "$HTTP" = "200" ] && [ "$ST" = "online" ] && [ "$PDF" = "404" ] && [ "$PAYROLL" = "401" ] && [ "$VERSION" = "1.22.3" ] && [ "$TOTALSCRIPTS" = "$EXPECT_TOTAL" ]; then
  ...
  mv unpacked unpacked.broken-v1223; mv unpacked.old unpacked
