#!/bin/bash
# API deploy v1.22.9 — finding batch: WP2 628 (AUDITOR reads + APPROVER pending-approvals read),
...
echo "== backup DB =="
TS=$(date +%Y%m%d-%H%M%S)
sudo -u postgres pg_dump teas | gzip > ~/backups/teas-pre-v1.22.9-deploy-$TS.sql.gz
gunzip -t ~/backups/teas-pre-v1.22.9-deploy-$TS.sql.gz && echo "DB_BACKUP_OK $(ls -la ~/backups/teas-pre-v1.22.9-deploy-$TS.sql.gz | awk '{print $5}')" || { echo "DB_BACKUP_FAILED -- abort"; exit 1; }

echo "== stage =="
rm -rf unpacked.new; mkdir unpacked.new
tar xf /tmp/teas-api-1.22.9-sc.tar -C unpacked.new
...
[ "$VERSION" = "1.22.9" ] && echo "PASS version=$VERSION" || echo "FAIL version=$VERSION"
...
if [ "$HTTP" = "200" ] && [ "$ST" = "online" ] && [ "$PDF" = "404" ] && [ "$APAGING" = "401" ] && [ "$VERSION" = "1.22.9" ] && [ "$TOTALSCRIPTS" = "$EXPECT_TOTAL" ] && [ "$AUDREADS" = "3" ]; then
  ...
  mv unpacked unpacked.broken-v1229; mv unpacked.old unpacked
