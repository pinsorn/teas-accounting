#!/bin/bash
# API deploy v1.22.2 — CN/DN reason Thai-label display fix (DocumentLabels.AdjustmentReasonLabel
# + TaxAdjustmentNoteService.Read.cs wiring). Display-only; DB storage (ReasonCode enum) untouched.
# ** NO new SqlScripts, NO new EF migrations this release. applied_sql_scripts MUST stay at 71. **
...
echo "== backup DB =="
TS=$(date +%Y%m%d-%H%M%S)
sudo -u postgres pg_dump teas | gzip > ~/backups/teas-pre-v1.22.2-deploy-$TS.sql.gz
gunzip -t ~/backups/teas-pre-v1.22.2-deploy-$TS.sql.gz && echo "DB_BACKUP_OK $(ls -la ~/backups/teas-pre-v1.22.2-deploy-$TS.sql.gz | awk '{print $5}')" || { echo "DB_BACKUP_FAILED -- abort"; exit 1; }

echo "== stage =="
rm -rf unpacked.new; mkdir unpacked.new
tar xf /tmp/teas-api-1.22.2-sc.tar -C unpacked.new
...
[ "$VERSION" = "1.22.2" ] && echo "PASS version=$VERSION" || echo "FAIL version=$VERSION"
...
if [ "$HTTP" = "200" ] && [ "$ST" = "online" ] && [ "$PDF" = "404" ] && [ "$PAYROLL" = "401" ] && [ "$VERSION" = "1.22.2" ] && [ "$TOTALSCRIPTS" = "$EXPECT_TOTAL" ]; then
  ...
  mv unpacked unpacked.broken-v1222; mv unpacked.old unpacked
