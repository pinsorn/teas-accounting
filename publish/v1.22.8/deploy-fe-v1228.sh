#!/bin/bash
# FE deploy v1.22.8 — finding batch WP1 (route-guard deny on 16 /new routes + CN/DN buttons +
...
# `git archive v1.22.8 frontend` overlay (--strip-components=1). Rebuild, restart, auto-rollback.
...
tar xf /tmp/fe-src-v1228.tar --strip-components=1
echo "overlaid: $(tar tf /tmp/fe-src-v1228.tar | wc -l) entries"
...
node node_modules/next/dist/bin/next build > /tmp/fe-build-v1228.log 2>&1
RC=$?
if [ $RC -ne 0 ]; then
  echo "BUILD_FAILED rc=$RC -- rolling back"
  tail -30 /tmp/fe-build-v1228.log
  mv .next .next.broken-v1228; mv .next.old .next
