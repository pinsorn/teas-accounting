#!/usr/bin/env bash
# TEAS API deploy — v2.2.0 (R3/H1 WP-4: the seven company-wide unique indexes)
#
...
echo "== PRECONDITION 3: the seven DROP targets exist =="
# migrationBuilder.DropIndex emits DROP INDEX with no IF EXISTS — a name mismatch is the same outage
# through a different door.
OLD=$(sudo -u postgres psql -d teas -tAc "
  SELECT count(*) FROM pg_indexes WHERE indexname IN (
    'ix_vendor_invoices_company_id_branch_id_doc_no','ix_tax_invoices_company_id_branch_id_doc_no',
    'ix_tax_adjustment_notes_company_id_branch_id_doc_no','ix_receipts_company_id_branch_id_doc_no',
    'ix_payment_vouchers_company_id_branch_id_doc_no','ix_fixed_assets_company_id_branch_id_doc_no',
    'ix_expense_claims_company_id_branch_id_doc_no');" | tr -d ' ')
[ "$OLD" = "7" ] || { echo "ABORT: expected 7 old indexes to drop, found $OLD -- DROP INDEX has no IF EXISTS"; exit 1; }
echo "PRECOND3_OK old_indexes=7"
