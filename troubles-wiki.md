...
Root-cause investigation (2026-07-19, specs/fix-s13-cf-edge-503.md): origin fully ruled out (zero 503s ever logged, no resource pressure, config clean). Top hypothesis = Cloudflare Bot Fight Mode / bot-scoring blocking the AUTOMATED browser session at the edge (curl sailed through the same minutes; same URLs pass+fail within one session = scoring, not a static rule). Fix lives in the CF dashboard (scoped WAF Skip rule) — needs Ham; full 8-item CF checklist in the spec.
...
## Full suite: `McpServerSmokeTests.E3_create_vendor_returns_id_code_name` fails with `result.IsError = true` — pre-existing, not your diff
Symptom: the ONLY failure in an otherwise-green full-suite run; `result.IsError.Should().NotBe(true)` at `McpServerSmokeTests.cs:1233` fails even when re-run in complete isolation (`--filter FullyQualifiedName~E3_create_vendor_returns_id_code_name`), so it is not xUnit collection-order flakiness.
Root cause: unconfirmed / not yet root-caused (open item). Confirmed PRE-EXISTING across at least two unrelated diffs: reproduces on a clean baseline with all pending changes stashed (2026-07-14, WP34 investigation) and again on an unrelated RLS-atomicity fix that never touches vendor creation (2026-07-18, `specs/fix-company-create-rls-atomic.md`).
Fix: do not treat this failure as a regression from your diff unless your diff touches vendor creation (`master.vendors`, the `create_vendor` MCP tool, or its handler chain). Note it in your gate evidence and move on; someone still needs to root-cause the underlying `create_vendor` MCP tool failure itself as a separate task.
Seen: 2026-07-14 (WP34), 2026-07-16 (backend suite run), 2026-07-18 (fix-company-create-rls-atomic.md gate — isolated re-run also failed, same error).
