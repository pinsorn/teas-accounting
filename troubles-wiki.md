...
## Line editor's "0% (ยกเว้น/ส่งออก)" option silently charges 7% on a VAT-registered company
- **Symptom:** user selects "0% (ยกเว้น/ส่งออก)" in the tax-rate/tax-code picker on a quotation, sales order, billing note, or tax invoice line for a VAT-registered company. The line is saved with that label; when the document is posted, 7% VAT appears instead of the expected zero-rated or exempt treatment.
- **Root cause:** `VAT0` is not in any company's `tax.tax_codes` master (it is a synthetic literal returned by `SalesLineBackstop.Resolve` for non-VAT companies only); selecting it on a VAT company causes the resolver to fall back to the standard-output tax code (VAT7 at 7%) because an unknown code is resolved as standard-rated. The control's label is honest ("0%"), but what it actually charges is not.
- **Fix:** two options — (1) **recommended, Package 2:** replace the rate-only dropdown with a real tax-code picker fed by the company's own master (11 output codes including 8 exempt categories), so users can select genuine exempt/zero-rated codes instead of a lying synthetic label. This is the only option that makes Thai VAT exemptions usable; (2) remove the "0%" option entirely from the rate dropdown for VAT companies and require customers to use a product with `DefaultOutputTaxCodeId` set to an exempt code instead.
- **Seen:** 2026-08-16, `specs/fix-chain-conversion-integrity.md` §10.4 (finding F14, confirmed in attempt log); no 422 or loss-of-data — user sees the VAT appear on the saved document, so it is visible, not a silent defect. Eight seeded exempt product categories (books, education, medical, agricultural, etc.) are unreachable from the UI until the real picker ships.

## A draft tax invoice created from a quotation cannot be edited or deleted from the browser
- **Symptom:** user converts an Accepted quotation to a draft tax invoice via the "สร้างใบกำกับภาษี" button. The draft appears in the list and can be viewed, but there is no edit button, no delete button, and no draft-edit page — the only options are to post it or abandon it.
...
## [FIXED 2026-09-05] Idempotency `F1b_Late_owner_after_full_takeover` — flaky B→409 `idempotency.in_progress` / deadlock was a cold-start harness race, NOT a middleware takeover bug

- **Seen:** 2026-09-05, blind acceptance test `IdempotencyDocumentFenceTests.F1b_...` (branch `gpt56-wpj-document-fence`). Skipped by the acceptance-tester who read it as a possible stale-takeover concurrency defect in the shipped claim-first `IdempotencyMiddleware`/`IdempotencyStore` (a probe showed a fresh NULL claim row that "nobody serviced" + B returning 409).
- **Symptom:** With A paused at the `IQuotationService` interface (a `TaskCompletionSource` gate) and A's claim back-dated past `StaleAfter`, request B (same key/body) intermittently returns `409 idempotency.in_progress` instead of taking the stale claim over (2xx); other runs deadlock outright.
- **Root cause (test-only):** The harness fired A, waited a fixed `Task.Delay(500)`, then asserted ONLY `taskA.IsCompleted == false` before back-dating A's claim. That assertion is satisfied by A merely being SLOW — the first request to a cold `WebApplicationFactory` pays JIT + DI-graph build + Npgsql pool warmup + the deliberately-slow API-key hash, easily >500 ms just to reach its claim INSERT. In that window no claim row exists yet, so `BackdateClaimAsync` hits **0 rows**, and A and B then race for the same key: whichever reaches the pipeline first claims and (`Notes == marker`) consumes the one-shot pause latch. Nondeterministic outcome. The instrumented run here DIRECTLY reproduced the **deadlock** (B won the latch, parked holding the only live claim, and the test's `await respB` never returned); the acceptance-tester's B→409 is the same race resolving the other way (A claims first, B then sees A's live NULL claim → InProgress → 409 after the 2 s wait). Either way no stale takeover occurs — the "fresh claim nobody serviced" the probe saw is consistent with a live claim, not a takeover artifact. `IdempotencyStore.ClaimAsync` / `IdempotencyMiddleware` are correct — the real stale-takeover path is exercised by T-F1 (external advisory lock) and by the fixed F1b.
- **Confirming trace:** instrumented F1b logged `after 500ms: taskA.IsCompleted=False` / `claim row BEFORE backdate: NO ROW` / `backdate rowcount = 0` — proving A had not claimed at the assertion point.
