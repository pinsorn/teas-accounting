...
## 2026-06-16 (cont. 98o — "ข wipe + onboarding + switcher + non-VAT + บท0 + version/squash + 2 test agents w/ PDFs" [Ham, orchestrate via subagents]) — **🚧 IN PROGRESS. Foundation เสร็จ+commit; Phase 4 manual rebuild กำลังรัน (subagents). main ยังที่ `87ee8c5` (program นี้อยู่บน feat branch, ยังไม่ push).**
...
- **🚧 Phase 4 manual rebuild (subagents, กำลังทำ):** recon เสร็จ (data-dependency map). clean DB มี master data co2/3 แต่ wipe เอา runtime demo ออก: **MD-EMP employees + payroll 202602** (ch6/07.06, 06.01 throw ถ้าไม่มี) + **foreign-vendor reverse-charge purchases** (ภ.พ.36/07.04). ch4-5 sales/purchase = walkthrough สร้างเองตอน capture. **agent 4a** (data rebuild) รันอยู่: seed master เป็น SQL idempotent (รอด wipe) + transactional ผ่าน API re-runnable + ไม่แตะ ม.70/ภ.ง.ด.54 (กัน pollution เดิม). ถัดไป: **4b capture pipeline** (sequential — shared servers/DB) + non-VAT chapters (03.03 person customer/04.11 nonvat E2E/05.06 person vendor) + ch0 wizard capture.
- **ค้าง:** Phase 4 capture · Phase 5 version control (MinVer+release-please) + migration squash (ผมเอง §6) · Phase 6 2 testing agents + PDFs. **ไม่ push origin** (Ham's call).
