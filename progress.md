...
- **⚠️⚠️ CLEANUP ITEM ให้ Ham — seed PV เปื้อน co2 live (ผมพลาด):** PV นี้เป็น **ค่าใช้จ่าย 50,000 บน FY2026** → ดัน P&L co2 จากกำไร 1,315.72 เป็น**ขาดทุน ~48,684**, ภ.พ.36 4→**5 แถว** (มี flag reverse-charge), tax-summary WHT +7,500. → **กระทบ tie-out ของ 07.03 CIT · 07.04 ภ.พ.36 · 08.01/08.02** ที่ผูกเลขไว้. **ลบไม่ได้สะอาด:** posted JE มี trigger `trg_je_no_delete_posted` (§4.2) + cert immutable + ไม่มี void endpoint. → **revert การแก้คู่มือทั้งหมดกลับ honest state (committed cont.98k)** เพื่อให้ของที่ commit สะอาด (ไม่ re-capture = manual ยังถูก). **co2 LIVE ยังเปื้อน.** **แนะนำ Ham:** (ก) **recreate accounting_dev จาก seed** (สะอาดสุด — PV นี้ add ตอน runtime ไม่ได้อยู่ใน seed script → DB ใหม่ไม่มี); (ข) reversing JE คืน P&L (sanctioned §4.2 แต่ partial — ไม่แก้ ภ.พ.36/ภ.ง.ด.54); (ค) เพิ่ม PV void/reverse feature. **บทเรียน: ห้าม post เอกสารการเงินจริงบน demo co ที่ตัวเลขเป็น load-bearing — เช็ค P&L blast radius ก่อน seed.**
