...
- ✅ **0-fill raster pass done same session (`fieldmap/zfill.py`, commit `b99ac2e`):** `Text661` = ช่องฐานภาษี [48-49] **ใบเดียวใช้ร่วม** กำไรสุทธิ/ขาดทุน/รายรับ (เลือกด้วย Group5 — ไม่มี box 46-47 แยกใน revision นี้); margin numbers confirm 662-672 = [50-51]…[61-62]; p1 จำนวนเงิน ชำระเพิ่มเติม=`Text2000-1`+`Text3`(สตางค์) [30-31] / ชำระไว้เกิน=`Text2000`+`Text3-2`. Crops ส่ง Ham แล้ว. **เหลือเปิด 1 เรื่อง:** radio choice↔option order ทุก group (Group1/5/21/6/7/8, Group00-06) — render-confirm รายตัวเป็น step แรกของ build session (pnd51 flip hazard).
