...
- **caveats (flag Ham):** (1) ใบแนบ pay-date column ว่าง (DTO ไม่มี per-row date); (2) "ใบแนบ จำนวน ราย/แผ่น" box ว่าง; (3) co2 ภ.ง.ด.53 ปน e2e-suite noise ในชื่อ payee (data ไม่ใช่ filler — filter ตอน manual sample); (4) ภ.ง.ด.54 amounts + ม.70 seed = follow-up.
- **ค้าง / ถัดไป:** **manual walkthroughs 07.07–07.10** (render-pdf-samples target + ch7 + gen-md/mkdocs — deferred batch, งานหนัก playwright) · ภ.ง.ด.54 ม.70 seed + amount verify · ใบแนบ pay-date (DTO enrich) + ราย/แผ่น count · ref-modal-buttons §3 (เพิ่มปุ่ม PDF).
