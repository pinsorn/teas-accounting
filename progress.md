...
- **Follow-up (Ham review):** (1) **i18n bug** — `status.Converted` (quotation filter) had no key → `FilterBar.statusLabel` logged a dev MISSING_MESSAGE (caught, but noisy). Audited every list page's `*_STATUSES` array vs `status.*` block: **Converted was the only gap** — added th `แปลงเป็นใบสั่งขาย` / en `Converted`. (2) **Product list** now surfaces the new attributes: a "การใช้งาน" filter (ทั้งหมด/ขายได้/ซื้อได้, reuses the BE `purpose` param via `useProducts(.,.,purpose)`) + per-row ขาย/ซื้อ badges + หน่วยธุรกิจ column (BU id→name via `useBusinessUnits`). i18n `product.businessUnit`. tsc 0, parity 0/0; quotations page now console-error-free.
