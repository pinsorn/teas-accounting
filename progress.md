...
- **#บท 7 ภาษี เริ่ม (2 walkthrough, read-only, commit `2a43458`+`3fce9a0`):** orient ก่อนเขียน (อ่าน source หน้า + ยืนยัน plan.md ว่า tax-filings UI ship จริง + co2 มีข้อมูลจาก บท 4–5). ทั้ง 2 = pattern เดียวกัน (period picker → **"แสดงตัวอย่าง" = คำนวณจาก GL ทันที ไม่ mutate**; ไม่กด "ยืนยัน/ปิดงวด" → re-capture ได้).
  ...
  - **07.02 ภ.ง.ด.3/53 (WHT remit)** (`/tax-filings` index + `/tax-filings/pnd53`) — index = catalog แบบ (PND30/3/53/54/36/51/CIT) + ประวัติยื่น; pnd53 preview = ตารางรวมใบ 50ทวิ ของงวด (นิติบุคคล; ภ.ง.ด.3 = บุคคล) นำส่ง ม.7 เดือนถัดไป, due 2026-07-07 ✓. **caption qualitative** (ไม่อ้าง row เจาะจง).
  - **⚠️ data-noise (flag Ham):** pnd53 ของ co2 **ปนใบ 50ทวิ จาก e2e suite** ("ผู้ขาย e2e จำกัด"/"purchase-chain e2e"/Amazon) → ตารางถูกต้องเชิงฟังก์ชันแต่ดู noisy สำหรับคู่มือจริง. **ลบไม่ได้** (50ทวิ ผูก PV posted = immutable §4.2). ทางแก้ที่สะอาด = capture บน demo company ใหม่/ล้าง หรือ filter — ไว้รอบ polish. (pnd30 ไม่กระทบ เพราะ aggregate.)
