...
- **ค้าง (Ham):** (1) **push** main to remote when ready (not pushed — release-please will pick up the feat/fix commits). (2) DOCS batch = subagent self-report only (not second-reviewed) — low risk. (3) optional precision for Sana: the openapi documents 204-returning lifecycle actions as 200 (file-wide convention); create-invoice doc says 201 vs code 200 (pre-existing).
