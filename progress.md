...
- **⚠️ ต้องให้ Ham เคาะ (flag ชัดใน matrix doc + 530 header):** (1) **SoD**: CHIEF_ACCOUNTANT + COMPANY_ADMIN ถือทั้ง PV create+approve (+ COMPANY_ADMIN ถือ PO create+approve) — admin/chief ถือครบเป็น pattern ปกติ (SoD บังคับด้วยการ assign role ต่างคนกัน) แต่รอ Ham ยืนยัน. (2) **`sys.role.manage`+`sys.user.manage`→COMPANY_ADMIN** (530 §D): ทำให้ per-company admin UI (Plan 1) ใช้ได้จริงโดย company-admin — bounded ด้วย ResolveTargetCompany (บริษัทตัวเอง) + SUPER_ADMIN assign ไม่ได้. ถ้า Ham อยากให้ admin = assign SUPER_ADMIN role แทน → ลบ §D. (3) `master.company.manage` แชร์ระหว่าง `/companies` (tax config, super-only §4.6) กับ `/company-profile/*` (company-admin แก้ข้อมูลตัวเอง) → company-admin แก้ profile ตัวเองไม่ได้โดยไม่ละเมิด §4.6 — profile-edit ควรมี perm แยก (finding).
