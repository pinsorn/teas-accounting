...
- **🔵 UX gap noted (not fixed):** a logged-out user opening an `approvalUrl` lands on `/login` WITHOUT a returnTo → after login they end up at the dashboard, not the draft. The login page already honours `returnTo` (built for OAuth consent) — the middleware cookie-gate just never sets it for dashboard paths. Candidate small fix next session.
