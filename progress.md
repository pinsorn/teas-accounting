...
- **ค้าง / residual (Ham):** (1) **real RD Prep import test** 1 รอบ (GUI) — ตอนนี้ **source-verified** (parser รับ no-header positional pipe) แต่ยังไม่เคยรัน GUI จริง (belt-and-suspenders; ยืนยัน ข้อ2/3 empty-when-0 + Branch HQ `"0"` vs `"00000"`). (2) ภ.ง.ด.50/51 = PDF เท่านั้น (ยื่นเว็บ/งบการเงิน DBD แยก XBRL — out of scope). (3) **dev login 401** (กระทบ live-smoke ทุก authed endpoint — แยกเรื่อง seed/cred). docs: `docs/RD-Forms/rd-prep-efiling-research.md` (+ importer parse-logic verified) + `docs/superpowers/specs/2026-06-20-rd-prep-wht-exporter.md` + spec PDFs.
