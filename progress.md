...
- **Known follow-ups (not blockers):** doc lists lost the old BU + date-range filters (kept status+party) — re-add BU as a select column-filter + a date-range control if Ham wants "ละเอียด" back. DataTable owns `<tr>` so per-row `opacity-50` (inactive) / `hover` styling dropped on masters (status text still shown). URL-persistence of filters not carried over (was on old FilterBar) — add if needed.
