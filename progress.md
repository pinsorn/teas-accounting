...
- **ค้าง (Ham):** (1) **commit**. (2) **B3 badge completion** — FE-2b skipped: `createdViaApiKey` is on list DTOs only, NOT the 6 detail DTOs → backend must project it onto detail responses, then FE shows AgentPendingBadge on detail (security gate is already backend-enforced; this is informational UX). (3) dashboard `pending-agent-approvals` DTO covers only TI/quotation/receipt — add PO/VI/PV breakouts (agentic can draft them). (4) openapi: undocumented routes flagged by docs subagent (quotation reject/convert-to-so, DO create-ti/create-invoice, etc.) — add to contract if wanted → Sana.
