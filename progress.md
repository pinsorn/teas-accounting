...
- **🔵 finding (flag Ham):** co2 `chart_of_accounts` **มีรหัสบัญชีซ้ำ** (น่าจะ COA seed รันซ้ำต่อ company) → ควรเช็คว่ามี `UNIQUE(company_id, account_code)` constraint ไหม. ถ้าไม่มี = product gap (ทุกบริษัทอาจมี COA ซ้ำได้). ยังไม่ได้ verify constraint — แค่สังเกตจาก TB.
