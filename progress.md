...
- **Payroll NEXT (P-A rest, fresh budget):** Employee **CRUD** (DTOs+validator+service+endpoints mirroring BusinessUnit) — needs a new **permission** `master.employee.manage` (add to `Permissions.cs` + `Permissions.All` + an idempotent seed SQL like `140_…` granting it to the owner role — the fiddly RBAC-seed step, do carefully) + FE master page + i18n. Then **P-C** run→payslip→GL (uses `ThaiPitCalculator`; SSO rate/ceiling from config), **P-D** outputs: monthly **per-employee payment-evidence/payslip PDF** (Ham requirement) + ภ.ง.ด.1/1ก (reuse `WhtBatchFormat` — PND1 same family) + employee 50ทวิ (`Wht50TawiFormFiller`).
