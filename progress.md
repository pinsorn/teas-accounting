...
- **Open (P-D #4):** SSO contribution file (separate format). Payroll core + ภ.ง.ด.1/1ก + payslip = COMPLETE. (decode `pnd1a_*` fields w/ DumpRects — adds employee address col; aggregate posted runs/year) · SSO file · the 3 WIP items above (esp. the radio-group fill extension). (compute from payslips → `TaxFilingStore`; batch file extend `WhtBatchFormat` — needs `FormatPND1V2_0.pdf` field list from rd.go.th) · ภ.ง.ด.1ก + employee 50ทวิ annual (`Wht50TawiFormFiller` FormType Pnd1) · SSO contribution file · FE payroll UI. 🟠 confirm 2569 SSO ceiling (config-only) before go-live.
