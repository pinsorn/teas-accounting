...
- **ปฏิเสธการออกเอกสารยื่นภาษีด้วยเลขผู้เสียภาษีที่ยังเป็นค่าเริ่มต้น** — แบบภาษี หนังสือรับรองหัก ณ ที่จ่าย
  (50 ทวิ) และไฟล์นำส่งประกันสังคม ออกไม่ได้จนกว่าจะตั้งค่าเลขประจำตัวผู้เสียภาษี 13 หลักของบริษัทเป็นค่าจริง
- **Audit trail แบบ append-only** ในทุกการเปลี่ยนสถานะเอกสาร พร้อมเหตุผลประกอบสำหรับการยกเลิก/ปฏิเสธ
  รองรับการตรวจสอบย้อนหลังตามพระราชบัญญัติการบัญชี
...
ทะเบียนทรัพย์สินถาวรพร้อมตารางค่าเสื่อมราคาแบบเส้นตรงรายเดือน เดือนแรกคำนวณตามสัดส่วนวันจริงที่ถือครอง
ตามประมวลรัษฎากร ม.65 ทวิ (2) เดือนถัดไปคิดเต็มเดือน ป้องกันการจำหน่ายสินทรัพย์หรือแก้ไขวันเริ่มคิดค่าเสื่อม
ย้อนหลังก่อนวันที่ได้มา และคำนวณตารางค่าเสื่อมใหม่ให้เมื่อแก้ไขอายุการใช้งาน/มูลค่าซาก โดยไม่กระทบยอด
ที่บันทึกบัญชีไปแล้ว

### เบิกค่าใช้จ่าย
วงจรอนุมัติเบิกจ่ายพนักงาน (ร่าง → ยื่นขอ → อนุมัติ → จ่ายแล้ว) แยกสิทธิ์ยื่นขอ/อนุมัติ/จ่ายเงินออกจากกัน
(segregation of duties — บทบาทที่ยื่นขอไม่จำเป็นต้องมีสิทธิ์อนุมัติหรือจ่าย) แต่ละบรรทัดระบุประเภทค่าใช้จ่าย
พร้อมกำกับว่าภาษีซื้อนำมาหักได้หรือเป็นภาษีต้องห้าม (เช่น ค่ารับรอง) ซึ่งจะถูกรวมเข้ากับยอดที่เบิกโดยอัตโนมัติ
เมื่อเป็นภาษีต้องห้าม บันทึกบัญชีอัตโนมัติเมื่อจ่ายเงิน

### กระทบยอดธนาคาร
นำเข้ารายการเดินบัญชีจากไฟล์ CSV ของ K Bank (KBiz) หรือไฟล์ PDF ของ K-Plus แล้วจับคู่กับรายการบัญชีด้วย
matching engine พร้อมบันทึกรายการปรับปรุง (เช่น ดอกเบี้ย ค่าธรรมเนียม) เป็น journal entry ได้ในหน้าจอเดียว
ยอดคงเหลือคำนวณแบบ deterministic จากรายการที่นำเข้า และการนำเข้าไฟล์ซ้ำ (superseded import) ลบรายการชุดเดิม
ให้อัตโนมัติเพื่อไม่ให้ยอดซ้อนกัน
...
และ onboarding wizard สำหรับการติดตั้งครั้งแรก ตัวอย่างการแบ่งสิทธิ์แบบละเอียด: การค้นหาพนักงาน
ในแบบฟอร์มเบิกค่าใช้จ่ายเปิดให้เห็นเฉพาะชื่อ-รหัสพนักงาน แยกสิทธิ์จากการเข้าถึงข้อมูลเงินเดือนเต็มรูปแบบ
...
พร้อม type-check frontend ในทุก pull request การ deploy จริงใช้ `docker-compose.coolify.yml`
(build image จาก Dockerfile ของ backend/frontend เชื่อมต่อ PostgreSQL ที่ Coolify จัดการแยกต่างหาก)
...
การตรวจสอบและแก้ไขด้านความปลอดภัยดำเนินต่อเนื่องทุก release (RBAC scope, MCP write authorization,
RLS บนตารางที่เพิ่มใหม่ ฯลฯ) — ดูรายการทั้งหมดที่ [CHANGELOG.md](CHANGELOG.md)
