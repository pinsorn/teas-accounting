...
| **Sprint 13c** ใหม่ | Spec ready (`Answer-Sana-Backend18.md`); blocked by Sprint 12 ship | **e-Tax production-readiness + Tier 1 mock infrastructure**. Per `docs/etax-environment-tiers.md` audit (3-tier swap matrix). 8 gaps + 4 drift cleanups: (1) config-key cleanup `Tax:EtaxEnabled/EtaxDeliveryEmailCc` removed + `ETaxBehaviorOptions.RdCcAddress` deleted (single source = `ETax:Email:RdCcAddress`); (2) `etax.submissions` append-only audit table (5-year legal retention); (3) `RedirectAllToEmail` + `WhitelistDomains` safety (CRITICAL for Tier 2 — prevents accidental customer-facing sends during UAT); (4) `IETaxXmlValidator` + ETDA มกค.14-2563 XSD schemas committed to `etax-schemas/`; (5) `IRdEfilingClient` + `MockRdEfilingClient` (Tier 1) + `RdHttpEfilingClient` skeleton (Tier 2/3 — real wiring deferred until UAT credentials provisioned); (6) in-process `IETaxSubmissionPipeline` + `ETaxRetryWorker` + backoff schedule + dead-letter; (7) dev tools: `gen-test-cert.sh` + `docker-compose.dev.yml` MailHog/MockServer + canned RD API responses; (8) CLAUDE.md "e-Tax environment switching" section. Result: Tier 1 → 2 → 3 = **config change only, zero code edit**. Estimate **~4-5 days**. |
| **Sprint 14** ใหม่ | Spec ready (`Answer-Sana-Backend19.md`); blocked by Sprint 12 + 13c ship | **External API Integration + Per-Key BU Binding**. Service-to-service API for microservices (Shopify, POS, internal apps). 8 phases: (P1) X-Api-Key middleware + ApiKey resolution (reuses Sprint 1-2 ApiKey entity) + claim population (company_id, scopes, default_business_unit_id, is_api_key=true); (P2) ApiKey CRUD endpoints + UI `/settings/api-keys` with plaintext-once display (Stripe pattern); (P3) `/api/v1/*` namespace mount (additive over existing root BFF routes — no breakage); (P4) Idempotency-Key middleware + `sys.idempotency_keys` table (24h TTL) + cleanup worker — REQUIRED for v1 mutations; (P5) Standard error envelope per plan §20.7 `{error: {code, message, details, trace_id, request_id}}` applied to v1 only (root keeps RFC 7807); (P6) ApiKey scope enforcement (PermissionAuthorizationHandler branches JWT user vs ApiKey scope check); (P7) **`ApiKey.DefaultBusinessUnitId` + auto-fill + lock + cross-BU receipt reject** — 1 microservice = 1 BU prefix (e.g. Reptify Shopify key → all TI auto = `MM-YYYY-TI-REPT-NNNN` without microservice knowing about BU); (P8) tests + OpenAPI spec update + 1 e2e simulating microservice integration. Auth isolation: v1 routes reject JWT, root routes reject ApiKey. Scope cuts: ❌ webhook outbound (Phase 2), ❌ rate limiting (Phase 2), ❌ OAuth (API key sufficient), ❌ cross-BU receipts via API key (defensive). Estimate **~6-7 days**. |
