...
## 2026-07-03 (cont. 122b — "approve-banner UX + e-Tax inert-UI cleanup + README rewrite → v1.10.1 released + deployed" [Ham, iterative]) — **✅ Three Ham asks landed and shipped to prod same day: (1) agent-approve UX (PR #34), (2) e-Tax buttons removed while the pipeline is inert + trust-boundary XML guard (PR #36), (3) README rewritten twice — v1.10 feature edition then a professional full-prose edition per Ham's feedback ("ไม่ใช่ pordee/caveman"). Released v1.10.1 + deployed (API 1.10.1.0 verified on the DLL, no migration, no env change, DB untouched).**

- **Agent approval flow demo (Ham asked to SEE it):** ran the real chain — minted an MCP key, `create_tax_invoice_draft` via `/mcp` (learned: line needs `taxCode`+`uomText`, `taxRate` is a FRACTION 0.07; Thai text via curl from a Windows console mojibakes to `???` — send JSON via `--data-binary @file` in UTF-8), opened the returned `approvalUrl` (`…/tax-invoices/{id}?action=approve`) in Playwright, screenshotted for Ham. Two demo drafts (#35 mojibake, #36 clean) remain in accounting_dev.
- **PR #34 — approve-banner UX (Ham: "งง มีสองปุ่ม"):** the yellow `?action=approve` card is now a flex row with the CTA pinned right, and the duplicate regular action (Post/Approve/Send) hides while the banner shows — on all 6 agent-draft pages (TI/receipt/quotation/PO/VI/PV); edit/delete/cancel stay. Verified live on draft #36.
- **PR #36 — e-Tax inert-UI cleanup (Ham's catch: "ยังไม่ได้ต่อ e-Filing ทำไมปุ่มโผล่"):** Sonnet read-only investigation traced both TI-detail buttons to the repo BASELINE commit, gated only on `sys.vatMode` — visible on every status incl. Draft; XML download built unsigned UBL with `<cbc:ID>DRAFT</cbc:ID>` (backend had NO status check); resend was a permanent no-op while `ETax:Enabled=false` but would email customer + cc `csemail@rd.go.th` if that flag ever flips. Fix: FE buttons REMOVED (restore plan in plan.md tech debt: gate on `ETax:Enabled` via `/system/info` + `Posted` + confirm dialog), BE `BuildXmlAsync` now throws `ti.not_posted` (mirrors ResendAsync). Gates: build 0/0 · TI/e-Tax 43/43 · tsc 0.
- **README:** two rewrites on Ham's order — first the v1.10 feature edition (badges, TEAS Connect, canonical paper DTO), then a professional full-prose edition (complete sentences, formal tone, e-Tax "ยังไม่เปิดใช้งาน" note, collapsible PowerShell block, MCP client table). Live on GitHub.
- **🚀 v1.10.1 released + deployed:** release PR #35 → tag → publish from tag at the real path (DLL stamps `1.10.1.0`) → pscp (md5-verified both sides — the cont.121 lesson) → API swap + FE overlay/build + pm2 restart. No migration/env/cert change. Verified: local api 401-up/web 307/mcp 401+challenge · external `/login` 200 + `/mcp` RFC 9728 challenge + discovery issuer over Cloudflare · DEPLOY-INFO → v1.10.1 · /tmp cleaned.
- **🔵 UX gap noted → ✅ FIXED same day (cont.122c, PR #37, v1.10.2 released + deployed):** the middleware cookie-gate now sets `returnTo=<original path+query>` on the /login redirect (root `/` stays bare; public paths untouched); the login page's existing `safeReturnTo()` open-redirect guard covers it — no new trust surface. Verified local + prod over Cloudflare (`/tax-invoices/1?action=approve` → 307 `/login?returnTo=%2Ftax-invoices%2F1%3Faction%3Dapprove`); prod DLL stamps `1.10.2.0`, DEPLOY-INFO bumped. ⚠️ footgun re-hit while testing: a STALE `next dev` still held :3000 serving OLD middleware (500s) while the fresh instance silently took :3001 — kill the old dev server before testing a middleware change [[stale-next-dev-no-hot-reload]].

## 2026-07-03 (cont. 122 — "merge + deploy the OAuth AS; check conflicts first; FE setup docs" [Ham]) — **✅ PR #31 merged → v1.10.0 RELEASED + PROD-DEPLOYED. Claude Mobile/Desktop native connectors can now OAuth into `https://teas.kazaki-rio.com/mcp`. Local browser E2E (the flagged #1 pre-deploy step) caught 3 REAL deploy blockers before they hit prod. DB backed up pre-migration; `oauth` schema applied clean; Ham's data intact.**

- **Conflict check (Ham-ordered):** merged main (v1.9.0 paper-DTO) into `feat/mcp-oauth` — code auto-merged; only `progress.md` (two parallel sessions both numbered cont.120/121 — OAuth entries relabeled `*-oauth`, all 4 kept newest-first) + the GENERATED rbac map (took main's, regenerated by the test suite). ⚠️ awk `'NR<6; NR>=34'` does NOT reorder (streams file order) — reassembled with sed ranges after one bungled attempt (restored via `git checkout -m`).
- **Local E2E (Playwright, :3000→:5080) — 3 blockers found+fixed (`6099232`):** (1) **discovery advertised the INTERNAL host** (OpenIddict derives endpoint URLs from the request host; prod would say `127.0.0.1:5180`) → `SetIssuer(App:BaseUrl)` + the `.well-known` passthroughs rewrite backend→public origin (`PUBLIC_BASE_URL`); (2) **`jwks_uri` had no public route** → new `/.well-known/jwks` passthrough; (3) **the `/mcp` passthrough (cont.119, pre-OAuth) hard-required X-Api-Key** → now forwards Bearer XOR X-Api-Key + emits the RFC 9728 challenge on its own 401. Full chain verified live: authorize→login(returnTo)→consent(company picker)→callback `code`+`iss`→token via passthrough→**Bearer `list_customers` returns data**→refresh rotation→bogus-key regression.
- **Prod-readiness code (same commit):** persistent-cert config `Oauth:{Signing,Encryption}CertPath`+`CertPassword` (X509CertificateLoader; **Production without certs now throws at boot** — never silent ephemeral); seeder redirect URIs extend via `Oauth:RedirectUris` config (client callback = config+restart, not redeploy); FE panel stale "Mobile in development" note → points at the native-connector OAuth section (th/en). Claude callbacks confirmed via WebSearch: `https://claude.{ai,com}/api/mcp/auth_callback`.
- **Gates:** build 0/0 · OAuth 23/23 ×2 · **full Api 563/0/8** (540+23) · tsc 0 · PR #31 CI green → squash-merge → main CI green → release PR #33 → **tag v1.10.0**.
- **Deploy (plink/pscp, plain .tar this time):** `pg_dump` backup FIRST (`teas-pre-oauth-20260703-082934.sql.gz`) → self-signed 10-yr signing+encryption `.pfx` pair generated server-side (`api/certs/`, chmod 600, password in `.secrets.env`) → run-api.sh gained `Oauth__*` env + run-web.sh `PUBLIC_BASE_URL` → API swap → **migration `20260701072643_AddOpenIddict` applied on boot** → FE rebuild. **🔴 one red round:** run-api.sh sources `$BASE/.secrets.env` (site root) but the deploy script appended `OAUTHCERTPASS` to `api/.secrets.env` → `set -u` unbound → api errored ~90s; moved the line, restarted, clean.
- **Verified over real Cloudflare:** `oauth-protected-resource` 200 (`resource=…/mcp`) · AS metadata issuer/authorize = public origin + S256 · jwks 200 · `POST /mcp` no-cred → **401 `WWW-Authenticate: Bearer resource_metadata=…`** · bogus X-Api-Key → backend `auth.invalid_api_key` (static-key path alive) · `/login` 200 · DEPLOY-INFO → v1.10.0; /tmp cleaned.
- **LEFT (needs Ham's Claude account):** add the connector in Claude Mobile/Desktop settings (`https://teas.kazaki-rio.com/mcp`), run login+consent, confirm a live tool call — the last unverifiable-by-me step. Deferred per gates doc: DCR, per-refresh membership revalidation (≤8h window), openapi `/oauth/*` delta (Sana).
