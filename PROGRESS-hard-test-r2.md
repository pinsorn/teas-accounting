...
## Phase 0 — stack boot: in progress

| Piece | State | Evidence |
|---|---|---|
| PostgreSQL 18 | running | `accounting_dev` up; 4 companies, 33 user_roles (round-1 state + co4) |
| API :5080 | booting | env overrides per `local-stack-boot-recipe`; DB already seeded → no reseed |
| FE :3000 | booting | fresh `npm run dev` (no stale-chunk risk — was down) |
...
|---|---|---|---|
| 1 | Payroll (WHT ภ.ง.ด.1/1ก, สปส., GL, closed-period) | sonnet | pending |
...
| 5 | co2 READ-ONLY tie-out + master integrity + report cross-check | sonnet | pending |
| 6 | Round-1 leftovers (co4 sale, N1/N2 live re-verify) | fold-in | pending |
