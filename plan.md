...
- **▶ TEAS Connect (MCP)** — see `docs/superpowers/specs/2026-07-01-mcp-connectivity-and-oauth-design.md` (cont.119).
  ...
  - ☑ **DEPLOY** — DONE cont.121/122: v1.9.0 shipped P1/P3/P4 (verified `/mcp` 401 JSON over Cloudflare); v1.10.0 shipped P2. DEPLOY-INFO on the server tracks the live version.
  - ☑ **P2 — OAuth 2.1+PKCE AS** — **BUILT (cont.120/121-oauth, PR #31) + MERGED + DEPLOYED v1.10.0 (cont.122, 2026-07-03, Ham "merge + deploy เลย")**. OpenIddict 7.5: RFC 9728/8414 discovery (public-origin rewrite via passthroughs + SetIssuer), authorize+consent (company picker), token+rotating refresh, Bearer XOR X-Api-Key on `/mcp` + 401 `WWW-Authenticate`, persistent X509 certs (prod fails fast without), config-driven redirect URIs (claude.ai/claude.com callbacks). Migration `AddOpenIddict` applied on prod (backup taken). Verified over Cloudflare. **Residual:** Ham adds the connector in Claude Mobile/Desktop and runs one live tool call; deferred = DCR (`/oauth/register`), per-refresh membership revalidation (≤8h), openapi `/oauth/*` delta (Sana).
