...
| **F** | LOW-01 | 2: `appsettings.Development.json`, `.gitignore` (+ optional 1-line `Directory.CreateDirectory(_root)` in `LocalDiskFileStorage` ctor) | same worker as C/D | none | `StorageRoot: ".attachments-dev"` (relative → under `backend/src/Accounting.Api` on `dotnet run`), gitignored. Env override unchanged. The optional eager `Directory.CreateDirectory` was shipped in `c4b4a56` and REVERTED 2026-09-05: it failed 351 tests on Linux CI (default root `/var/teas` unwritable; invisible on Windows) — troubles-wiki entry. |
...
| **J** (follow-up, Codex review 2026-09-05 F1/F2 — Ham's scope call) | Document-level idempotency fence: the claim id protects the idempotency ROW, not the business commit, so two accepted windows remain (owner still running after `StaleAfter` gets taken over → both commit; crash between business commit and `CompleteAsync` → retry after `StaleAfter` re-executes). Fix = persist `(api_key_id, idempotency_key)` ON the document (quotations / tax_invoices / receipts, nullable + partial UNIQUE) inside the service's own transaction, and have the takeover path look the document up first (replay it instead of executing). Lite option: `AddRequestTimeouts` (~120 s) on `/api/v1` bounds F1 only. | 3 services + 3 DDL migrations + middleware hook + tests | opus-designer → Sonnet → acceptance-tester (Codex's two regression tests) → Opus | integration | **SHIPPED 2026-09-05 — PR #120 merged (`3675144`).** Blind 32/0/0, Tier-3 1401/0/14, Opus APPROVE ×2, Codex rounds 1+2 closed (J9 delete-releases-key; T-F1 deterministic). Spec I11 superseded. |
